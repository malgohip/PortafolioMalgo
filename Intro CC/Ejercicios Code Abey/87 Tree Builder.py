...
"""
"""
a = open('Ejercicios Code Abey\TXTs\e87 Tree Builder.txt','r')
P= a.readlines()
...
print(nodos)
"""
nodos=[11, 25, 27, 21, 17, 10, 26, 1, 8, 12, 6, 19, 23, 9, 5, 2, 22, 20, 7, 24, 16, 18, 4, 3, 14, 15, 13]

def ramas(n):
    arbol=['-',n,'-']
    return arbol

def check_rama(arbol,n):
    if n > arbol[1]:
        if arbol[2] == '-':
            arbol[2]=ramas(n)
        else:
            arbol[2]=check_rama(arbol[2],n)
    elif n < arbol[1]:
        if arbol[0] == '-':
            arbol[0]=ramas(n)
        else:
            arbol[0]=check_rama(arbol[0],n)
    return arbol
...
for x in range(len(nodos)):
    if x == 0:
        ...
        arbol=check_rama(arbol,nodos[x])
respuesta=str(arbol)
